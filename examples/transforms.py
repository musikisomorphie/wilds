...
def initialize_image_resize_and_center_crop_transform(config, dataset):
    ...
    scaled_resolution = tuple(int(res*config.resize_scale)
                              for res in dataset.original_resolution)
    ...
    if is_training:
        transforms_ls = [
            ...
            transforms.ColorJitter(
                brightness=0.8, contrast=0.8, saturation=0.8, hue=0.1),
            transforms.ToTensor()]
